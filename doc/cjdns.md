# CJDNS support in ElevenSeventyFive Core

It is possible to run ElevenSeventyFive Core over CJDNS, an encrypted IPv6 network that
...
done outside of ElevenSeventyFive Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the ElevenSeventyFive network and individual nodes.
...
before it will work with your ElevenSeventyFive Core node. This is described in steps
...
## Run ElevenSeventyFive Core with CJDNS

Once you are connected to the CJDNS network, the following ElevenSeventyFive Core
...
When enabled, this option tells ElevenSeventyFive Core that it is running in an
...
IPv6 local network. This helps ElevenSeventyFive Core perform better address management:
...
CJDNS support was added to ElevenSeventyFive Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `elevenseventyfive-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/elevenseventyfive/elevenseventyfive/blob/master/doc/tor.md) for
details.

## CJDNS-related information in ElevenSeventyFive Core

There are several ways to see your CJDNS address in ElevenSeventyFive Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `elevenseventyfive-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `elevenseventyfive-cli getpeerinfo`).
