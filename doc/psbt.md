# PSBT Howto for ElevenSeventyFive Core

Since ElevenSeventyFive Core 0.17, an RPC interface exists for Partially Signed ElevenSeventyFive
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/elevenseventyfive/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for ElevenSeventyFive transactions that are not fully signed
...
## PSBT in ElevenSeventyFive Core
...
#### Multisig with multiple ElevenSeventyFive Core instances
...
ElevenSeventyFive Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `elevenseventyfive-cli` is used.
...
Later, when *V* ESF has been received on *Amulti*, and Bob and Carol want to
