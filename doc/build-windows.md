...
Below are some notes on how to build ElevenSeventyFive Core for Windows.

The options known to work for building ElevenSeventyFive Core on Windows are:
...
Note that for WSL the ElevenSeventyFive Core source path MUST be somewhere in the default mount file system, for
example /usr/src/elevenseventyfive, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\elevenseventyfive`, for example:

    make install DESTDIR=/mnt/c/workspace/elevenseventyfive
