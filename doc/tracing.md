# User-space, Statically Defined Tracing (USDT) for ElevenSeventyFive Core

ElevenSeventyFive Core includes statically defined tracepoints to allow for more
...
======================
                ...
                ┌──────────────────┐            ┌──────────────┐
                │ tracing script   │            │ elevenseventyfived     │
                │==================│      2.    │==============│
                │  eBPF  │ tracing │      hooks │              │
...
**Note**: ElevenSeventyFive Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to ElevenSeventyFive Core
...
Multiple tools can list the available tracepoints in a `elevenseventyfived` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in ElevenSeventyFive Core, use `info probes` in `gdb`:

```
$ gdb ./src/elevenseventyfived
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/elevenseventyfived
stap net        outbound_message 0x0000000000107c05 /src/elevenseventyfived
stap validation block_connected  0x00000000002fb10c /src/elevenseventyfived
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in ElevenSeventyFive Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/elevenseventyfived | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./src/elevenseventyfived -v
