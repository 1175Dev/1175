...
""" Example logging ElevenSeventyFive Core mempool events using the mempool:added,
...
def main(elevenseventyfived_path):
    elevenseventyfived_with_usdts = USDT(path=str(elevenseventyfived_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    elevenseventyfived_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    elevenseventyfived_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    elevenseventyfived_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    elevenseventyfived_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[elevenseventyfived_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/elevenseventyfived")
        exit(1)
