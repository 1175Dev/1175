...
This directory contains the files necessary to perform bootstrappable ElevenSeventyFive
Core builds.
...
To build ElevenSeventyFive Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      ElevenSeventyFive Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/elevenseventyfive-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/elevenseventyfive-core/guix.sigs).
