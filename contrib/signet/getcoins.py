...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing elevenseventyfive-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='elevenseventyfive-cli', help='elevenseventyfive-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='ElevenSeventyFive address to which the faucet should send')
...
parser.add_argument('elevenseventyfive_cli_args', nargs='*', help='Arguments to pass on to elevenseventyfive-cli (default: -signet)')

args = parser.parse_args()

if args.elevenseventyfive_cli_args == []:
    args.elevenseventyfive_cli_args = ['-signet']


def elevenseventyfive_cli(rpc_command_and_params):
    argv = [args.cmd] + args.elevenseventyfive_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = elevenseventyfive_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = elevenseventyfive_cli(['getnewaddress', 'faucet', 'bech32'])
