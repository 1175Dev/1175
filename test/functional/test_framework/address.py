...
# NOTE (1175): these keep their upstream ADDRESS_BCRT1_* names (so imports don't break) but
# hold the 1175 regtest equivalents (HRP "resf"), re-encoded from the same witness programs.
ADDRESS_BCRT1_UNSPENDABLE = 'resf1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqlwcrre'
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = 'addr(resf1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqlwcrre)#gvxe6dy7'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_BCRT1_P2WSH_OP_TRUE = 'resf1qft5p2uhsdcdc3l2ua4ap5qqfg4pjaqlp250x7us7a8qqhrxrxfsqrqeata'
...
def create_deterministic_address_bcrt1_p2tr_op_true(explicit_internal_key=None):
    ...
    if explicit_internal_key is None:
        assert_equal(address, 'resf1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqs20e965')
...
def program_to_witness(version, program, main=False):
    ...
    return encode_segwit_address("esf" if main else "resf", version, program)  # 1175 HRPs
...
def bech32_to_bytes(address):
    ...
    if hrp not in ['esf', 'tesf', 'sesf', 'resf']:  # 1175 HRPs (main/testnet/signet/regtest)
        return (None, None)
...
def address_to_scriptpubkey(address):
    ...
    if version in (100, 51):  # 1175 regtest / main pubkey hash
        return keyhash_to_p2pkh_script(payload)
    elif version in (196, 50):  # 1175 regtest/testnet / main script hash
