    ...
    def send_blocks_and_test(self, blocks, node, *, success=True, force_send=False, reject_reason=None, expect_disconnect=False, timeout=60, is_decoy=False):
        ...
        with node.assert_debug_log(expected_msgs=reject_reason):
            ...
            if force_send:
                ...
                # 1175: announce headers in batches no larger than MAX_HEADERS_RESULTS (800);
                # a single larger headers message is treated as misbehavior and disconnects us.
                all_headers = [CBlockHeader(block) for block in blocks]
                for i in range(0, len(all_headers), MAX_HEADERS_RESULTS):
                    self.send_message(msg_headers(all_headers[i:i + MAX_HEADERS_RESULTS]))
                self.wait_until(
