...
MAX_HEADERS_RESULTS = 800  # Number of headers the node SENDS in one getheaders result, and the "full batch" continuation threshold (lowered from Bitcoin's 2000 so a full batch of AuxPoW headers stays under MAX_PROTOCOL_MESSAGE_LENGTH; must match the node's MAX_HEADERS_RESULTS)
MAX_HEADERS_RESULTS_ACCEPT = 2000  # Maximum number of headers the node ACCEPTS in one headers message before Misbehaving (decoupled from and larger than MAX_HEADERS_RESULTS for wire-compat with legacy 2000-header IBD batches; must match the node's MAX_HEADERS_RESULTS_ACCEPT)
...
MAGIC_BYTES = {
    "mainnet":  b"\x74\xba\x1b\xe4",  # ESF mainnet  (chainparams.cpp CMainParams)
    "testnet3": b"\x1e\x3c\xa7\x46",  # ESF testnet3 (chainparams.cpp CTestNetParams)
    "regtest":  b"\x64\x3f\x9c\x8f",  # ESF regtest  (chainparams.cpp CRegTestParams)
    # Signet magic = sha256d(signet_challenge)[:4].  ESF uses the same default
    # challenge as Bitcoin signet (512103ad5e...52ae), so the magic is identical.
...
# AuxPoW constants (must match src/primitives/block.h)
BLOCK_VERSION_AUXPOW = (1 << 8)
BLOCK_VERSION_CHAIN_START = (1 << 16)


class CAuxPow:
    """Merged mining proof attached to an AuxPoW block."""
...
class CBlock(CBlockHeader):
    __slots__ = ("vtx", "auxpow")

    def __init__(self, header=None):
        super().__init__(header)
        self.vtx = []
        self.auxpow = None  # CAuxPow if BLOCK_VERSION_AUXPOW is set, else None
    ...
    def serialize(self, with_witness=True):
        r = b""
        r += super().serialize()
        if (self.nVersion & BLOCK_VERSION_AUXPOW) and self.auxpow is not None:
...
class msg_headers:
    ...
    def serialize(self):
        ...
        for x in self.headers:
            b = CBlock(x)
            # CBlock(x) resets auxpow to None; carry it over so merge-mined headers
            # serialize their proof (matches CBlockHeader carrying auxpow on the wire).
            b.auxpow = getattr(x, "auxpow", None)
            blocks.append(b)
