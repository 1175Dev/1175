...
TMPDIR_PREFIX = "elevenseventyfive_func_test_"
...
class ElevenSeventyFiveTestMetaClass(type):
    """Metaclass for ElevenSeventyFiveTestFramework.

    Ensures that any attempt to register a subclass of `ElevenSeventyFiveTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ElevenSeventyFiveTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ElevenSeventyFiveTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ElevenSeventyFiveTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ElevenSeventyFiveTestFramework(metaclass=ElevenSeventyFiveTestMetaClass):
    """Base class for a elevenseventyfive test script.

    Individual elevenseventyfive test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave elevenseventyfiveds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop elevenseventyfiveds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use elevenseventyfive-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_elevenseventyfived = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "elevenseventyfived" + config["environment"]["EXEEXT"],
        )
        fname_elevenseventyfivecli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "elevenseventyfive-cli" + config["environment"]["EXEEXT"],
        )
        fname_elevenseventyfiveutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "elevenseventyfive-util" + config["environment"]["EXEEXT"],
        )
        self.options.elevenseventyfived = os.getenv("ELEVENSEVENTYFIVED", default=fname_elevenseventyfived)
        self.options.elevenseventyfivecli = os.getenv("ELEVENSEVENTYFIVECLI", default=fname_elevenseventyfivecli)
        self.options.elevenseventyfiveutil = os.getenv("ELEVENSEVENTYFIVEUTIL", default=fname_elevenseventyfiveutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: elevenseventyfiveds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("ElevenSeventyFiveRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'elevenseventyfived', self.options.elevenseventyfived) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'elevenseventyfive-cli', self.options.elevenseventyfivecli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                elevenseventyfived=binary[i],
                elevenseventyfive_cli=binary_cli[i],
                version=versions[i],
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple elevenseventyfiveds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a elevenseventyfived test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple elevenseventyfived test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as elevenseventyfived's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    elevenseventyfived=self.options.elevenseventyfived,
                    elevenseventyfive_cli=self.options.elevenseventyfivecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
    ...
    def skip_if_no_elevenseventyfived_tracepoints(self):
        """Skip the running test if elevenseventyfived has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("elevenseventyfived has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_elevenseventyfived_zmq(self):
        """Skip the running test if elevenseventyfived has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("elevenseventyfived has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if elevenseventyfive-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("elevenseventyfive-wallet has not been compiled")

    def skip_if_no_elevenseventyfive_util(self):
        """Skip the running test if elevenseventyfive-util has not been compiled."""
        if not self.is_elevenseventyfive_util_compiled():
            raise SkipTest("elevenseventyfive-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if elevenseventyfive-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("elevenseventyfive-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether elevenseventyfive-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether elevenseventyfive-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_elevenseventyfive_util_compiled(self):
        """Checks whether elevenseventyfive-util was compiled."""
        return self.config["components"].getboolean("ENABLE_ELEVENSEVENTYFIVE_UTIL")
