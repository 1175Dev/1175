...
# 1175 regtest genesis block nTime (blocktools.TIME_GENESIS_BLOCK is Bitcoin's
# regtest genesis time and would place mocktime far before 1175's genesis,
# forcing each mined block's timestamp far into the future -> "time-too-new").
TIME_GENESIS_BLOCK = 1769468775
# 1175 regtest: nPowTargetTimespan (1175*10*60) / nPowTargetSpacing (10*60) = 1175.
# The BIP94 timewarp bump (and this test's retarget-boundary math) keys off the
# node's DifficultyAdjustmentInterval(), which is 1175 here, not Bitcoin's value.
DIFFICULTY_ADJUSTMENT_INTERVAL = 1175
...
class MiningTest(BitcoinTestFramework):
    ...
    def mine_chain(self):
        ...
        # 1337 (0x539) sets bit 8, which is 1175's BLOCK_VERSION_AUXPOW flag and
        # would require auxpow data. Use 1234 (0x4D2) instead: an arbitrary custom
        # version with the auxpow bit clear, preserving the pass-through check.
        self.restart_node(0, extra_args=[f'-mocktime={t}', '-blockversion=1234'])
        self.connect_nodes(0, 1)
        assert_equal(1234, self.nodes[0].getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)['version'])
