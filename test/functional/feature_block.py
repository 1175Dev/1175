...
class FullBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # 1175 (ESF-AUXPOW-002, src/net_processing.cpp): a block message that fails to
        # deserialize (here a non-canonical ReadCompactSize) is penalised and the peer is
        # disconnected, rather than merely logged as upstream does. Expect the disconnect;
        # send_blocks(reconnect=True) reconnects a fresh peer afterwards.
        self.send_blocks([b64a], success=False, reject_reason='non-canonical ReadCompactSize()', reconnect=True)
        ...
        # 1175: keep the large-reorg chains just under MAX_HEADERS_RESULTS (800). Upstream uses
        # 1088, but 1175 lowered the P2P headers limit to 800 (M3 auxpow-size cap). A >800-block
        # chain must be announced in multiple headers messages, which puts the node into its
        # two-phase low-work headers sync (presync + redownload); that path does not download an
        # equal-work competing chain, and the P2PDataStore test helper cannot drive its redownload
        # phase (non-continuous headers). At <=799 the whole chain is one headers message that the
        # node commits directly, so the equal-work alt chain is downloaded and the re-org still works.
        self.log.info("Test a re-org of a large number of blocks (799 blocks)")

        self.move_tip(88)
        LARGE_REORG_SIZE = 799
        blocks = []
        spend = out[32]
