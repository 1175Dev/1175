...
class P2PIBDStallingTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # 1175: MAX_HEADERS_RESULTS is 800 (M3 auxpow size cap), so the 1024-header
        # lookahead must be announced across several headers messages -- a single
        # >800-header message is treated as misbehavior and disconnects the peer.
        lookahead_headers = [CBlockHeader(b) for b in blocks[:NUM_BLOCKS-1]]
        peers = []
        ...
        for id in range(NUM_PEERS):
            ...
            self.send_headers_in_batches(peers[-1], lookahead_headers)
        ...
        full_headers = [CBlockHeader(b) for b in blocks]
        with node.assert_debug_log(expected_msgs=['Stall started']):
            for p in peers:
                self.send_headers_in_batches(p, full_headers)
            self.all_sync_send_with_ping(peers)
    ...
    def send_headers_in_batches(self, peer, headers):
        # Announce headers in chunks no larger than MAX_HEADERS_RESULTS so the node
        # does not treat the message as misbehavior (1175 lowered the limit to 800).
        for i in range(0, len(headers), MAX_HEADERS_RESULTS):
            msg = msg_headers()
            msg.headers = headers[i:i + MAX_HEADERS_RESULTS]
