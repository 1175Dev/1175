...
class BaseNode(P2PInterface):
    def send_header_for_blocks(self, new_blocks):
        # Send in batches of at most MAX_HEADERS_RESULTS: 1175's AuxPoW headers are larger,
        # so the node caps a headers message below Bitcoin's 2000 and would disconnect a
        # peer that sends more than MAX_HEADERS_RESULTS headers in one message.
        for i in range(0, len(new_blocks), MAX_HEADERS_RESULTS):
            headers_message = msg_headers()
            headers_message.headers = [CBlockHeader(b) for b in new_blocks[i:i + MAX_HEADERS_RESULTS]]
