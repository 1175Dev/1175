...
libs = [
    'libelevenseventyfive_cli',
    'libelevenseventyfive_common',
    'libelevenseventyfive_crypto',
    'libelevenseventyfive_node',
    'libelevenseventyfive_util',
    'libelevenseventyfive_wallet_tool',
    'libelevenseventyfive_wallet',
    'libelevenseventyfive_zmq',
    'bench_elevenseventyfive',
    'libtest_util',
]
...
def parse_config_into_esf_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"ElevenSeventyFive Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/elevenseventyfive_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/elevenseventyfive_config.h'), "w", encoding="utf8") as esf_config:
        esf_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='ElevenSeventyFive-core msbuild configuration initialiser.')
    ...
    parse_config_into_esf_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/elevenseventyfive_config.h'), os.path.join(SOURCE_DIR, 'config/elevenseventyfive-config.h'))
