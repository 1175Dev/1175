...
static void SetupElevenSeventyFiveTxArgs(ArgsManager &argsman)
{
...
{
    SetupElevenSeventyFiveTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " elevenseventyfive-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  elevenseventyfive-tx [options] <hex-tx> [commands]  Update hex-encoded elevenseventyfive transaction\n"
                "or:     elevenseventyfive-tx [options] -create [commands]   Create hex-encoded elevenseventyfive transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded elevenseventyfive transaction
