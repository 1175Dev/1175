...
/**
 * ASERT (aserti3-2d) — exact BCHN implementation.
 * Computes next target as a function of anchor target, time elapsed, height diff.
 * Half-life of 172800s (2 days) for mainnet.
 */
...
/** Validate AuxPoW attached to a merged-mined block.
 *  Takes a CBlockHeader (not CBlock) so it can be used at the header layer
 *  (HEADERS-message anti-DoS gate) as well as on full blocks — it only needs
 *  the header fields (nVersion, nBits, GetHash) and the attached auxpow. */
