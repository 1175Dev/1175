...
const char * const ELEVENSEVENTYFIVE_CONF_FILENAME = "elevenseventyfive.conf";
const char * const ELEVENSEVENTYFIVE_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //elevenseventyfive-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", ELEVENSEVENTYFIVE_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\ElevenSeventyFive
    // macOS: ~/Library/Application Support/ElevenSeventyFive
    // Unix-like: ~/.elevenseventyfive
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ElevenSeventyFive";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/ElevenSeventyFive";
#else
    // Unix-like
    return pathRet / ".elevenseventyfive";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", ELEVENSEVENTYFIVE_CONF_FILENAME));
}
