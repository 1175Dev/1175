                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // AuxPoW proof-of-work and activation-height are not enforced by CheckBlock
    // (which is context-free) or by ContextualCheckBlock (which ConnectBlock does
    // not invoke, and which -reindex-chainstate skips entirely).  Re-enforce both
    // here so that corrupt on-disk data is detected during any ConnectBlock path
    // including reindex, chain reconnection after a restart, and -reindex-chainstate.
    if (!fJustCheck && (block.nVersion & BLOCK_VERSION_AUXPOW)) {
        const int nHeight = pindex->nHeight;
        if (!params.GetConsensus().IsAuxpowActive(nHeight)) {
            state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "auxpow-too-early",
                strprintf("AuxPoW block at height %d before activation height %d",
                          nHeight, params.GetConsensus().nAuxpowStartHeight));
            return FatalError(m_chainman.GetNotifications(), state, _("Corrupt pre-activation AuxPoW block found; shutting down"));
        }
        if (!CheckAuxProofOfWork(block, params.GetConsensus())) {
            return FatalError(m_chainman.GetNotifications(), state, _("Corrupt AuxPoW block found indicating potential hardware failure; shutting down"));
        }
    }
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    // AuxPoW block PoW is verified in ContextualCheckBlock (after the activation
    // height guard in ContextualCheckBlockHeader has already passed).
    if (fCheckPOW && !(block.nVersion & BLOCK_VERSION_AUXPOW)) {
...
bool HasValidProofOfWork(const std::vector<CBlockHeader>& headers, const Consensus::Params& consensusParams)
{
    return std::all_of(headers.cbegin(), headers.cend(),
        [&](const auto& header) {
            // AuxPoW blocks are PoW-solved via the parent block hash. The HEADERS
            // message now carries the auxpow proof alongside the header, so verify
            // it here at the header layer: this makes an auxpow-flagged header cost
            // real parent-chain PoW to produce, closing the zero-cost header-flood
            // DoS. A header with the bit set but no/invalid auxpow fails this check,
            // and CheckHeadersPoW then bans the peer.
            if (header.nVersion & BLOCK_VERSION_AUXPOW) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // AuxPoW activation height enforcement:
    // - Before activation: reject any block claiming to be merged-mined
    // - After activation: accept both AuxPoW and solo-mined blocks
    //   (solo mining remains valid; mandatory AuxPoW is not enforced
    //   to protect chain liveness on a small network with few miners)
    const bool fAuxpowBitSet = (block.nVersion & BLOCK_VERSION_AUXPOW) != 0;
    if (fAuxpowBitSet && !consensusParams.IsAuxpowActive(nHeight))
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // AuxPoW activation and proof checks.
    // The activation height check here is redundant with ContextualCheckBlockHeader
    // in the normal AcceptBlock path.  It exists for two reasons:
    //   1. -reindex-chainstate skips ContextualCheckBlock entirely (see note above
    //      this function), so without this check a pre-activation AuxPoW block
    //      injected via DB corruption would not have its height enforced during
    //      a chainstate-only reindex.
    //   2. Defense-in-depth against any future code path that calls AcceptBlock
    //      without first passing through ContextualCheckBlockHeader.
    if (block.nVersion & BLOCK_VERSION_AUXPOW) {
        if (!consensusParams.IsAuxpowActive(nHeight))
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER,
                ...
                strprintf("AuxPoW block at height %d before activation height %d",
                          nHeight, consensusParams.nAuxpowStartHeight));
        // The auxpow proof is NOT committed to the block hash (GetHash covers only the
        // pure header fields plus the AUXPOW bit). A missing/invalid proof on a body is
        // therefore a mutation of an otherwise-valid block, exactly like witness/merkle
        // malleability: report BLOCK_MUTATED, not BLOCK_INVALID_HEADER, so AcceptBlock
        // does NOT set BLOCK_FAILED_VALID on this hash. Otherwise a peer could send a body
        // with the same hash but a corrupted proof to permanently poison the entry and get
        // the honest block rejected as duplicate-invalid. (AcceptBlockHeader already gates
        // an attached bad proof before an index entry exists, and ConnectBlock re-checks the
        // proof for -reindex-chainstate; this body check is defense-in-depth for any future
        // AcceptBlock path that bypasses AcceptBlockHeader — without the poisoning.)
        if (!block.auxpow)
            return state.Invalid(BlockValidationResult::BLOCK_MUTATED, "bad-auxpow", "AuxPoW bit set but no auxpow data");
        if (!CheckAuxProofOfWork(block, consensusParams))
            return state.Invalid(BlockValidationResult::BLOCK_MUTATED, "bad-auxpow", "AuxPoW check failed");
    }
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        // Non-AuxPoW blocks: the direct-hash PoW is checked in CheckBlockHeader.
        // AuxPoW blocks skip the direct-hash check here (the block hash is not the proof);
        // their aux proof is verified below, AFTER ContextualCheckBlockHeader applies the
        // activation-height rule, so a pre-activation auxpow header is reported as
        // "auxpow-too-early" and not "bad-auxpow".
        const bool fCheckHeaderPoW = !(block.nVersion & BLOCK_VERSION_AUXPOW);
        if (!CheckBlockHeader(block, state, GetConsensus(), fCheckHeaderPoW)) {
        ...
        // Verify the AuxPoW proof at header admission, before any block-index entry is
        // created for this hash. Runs AFTER ContextualCheckBlockHeader so the
        // activation-height rule (auxpow-too-early) takes precedence. Gated on an actually
        // ATTACHED proof (block.auxpow != null): the `block` and `cmpctblock` paths — which
        // would otherwise admit an unverified proof off claimed work — always carry it, so
        // this still gates them. Headers reconstructed by the low-work headers-sync
        // redownload carry the AUXPOW bit but a null auxpow (their proof was already
        // verified on the wire by CheckHeadersPoW/HasValidProofOfWork before being stripped
        // for buffering); re-rejecting those here would stall mainnet IBD. A `block` that
        // somehow reaches AcceptBlock with a null/invalid proof is still caught by the
        // body-level ContextualCheckBlock check (as BLOCK_MUTATED).
        if ((block.nVersion & BLOCK_VERSION_AUXPOW) && block.auxpow && !CheckAuxProofOfWork(block, GetConsensus())) {
            LogDebug(BCLog::VALIDATION, "%s: AuxPoW proof-of-work failed for new header %s\n", __func__, hash.ToString());
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-auxpow", "AuxPoW check failed at header admission");
        }
