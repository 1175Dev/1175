...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " elevenseventyfive-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The elevenseventyfive-tx tool is used for creating and modifying 1175 transactions.\n\n"
                "elevenseventyfive-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded 1175 transaction, or with \"-create [commands]\" to create a hex-encoded 1175 transaction.\n"
                "\n"
                "Usage: elevenseventyfive-tx [options] <hex-tx> [commands]\n"
                "or:    elevenseventyfive-tx [options] -create [commands]\n"
                "\n";
