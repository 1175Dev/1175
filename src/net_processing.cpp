...
/** Bounded cache of AuxPoW proofs by block hash (FIFO eviction), used only for header relay.
 *  AuxPoW is not stored in the block index, so serving an auxpow-flagged header would
 *  otherwise require reading the whole block body from disk every time. Caching the proof
 *  serves repeat/steady-state relay without disk I/O. A cold cache still costs one full-block
 *  read per uncached auxpow header served — the same I/O an honest IBD peer needs — which is
 *  intentionally NOT capped or truncated, because a short headers batch would be read by the
 *  requester as end-of-chain and abort its sync. */
namespace {
class AuxpowRelayCache
{
    mutable Mutex m_mutex;
    struct Hasher { size_t operator()(const uint256& h) const noexcept { return static_cast<size_t>(h.GetUint64(0)); } };
    std::unordered_map<uint256, std::shared_ptr<CAuxPow>, Hasher> m_map GUARDED_BY(m_mutex);
    std::deque<uint256> m_order GUARDED_BY(m_mutex);
    static constexpr size_t MAX_ENTRIES = 4096;
public:
    std::shared_ptr<CAuxPow> Get(const uint256& hash) const
    {
        LOCK(m_mutex);
        auto it = m_map.find(hash);
        return it == m_map.end() ? nullptr : it->second;
    }
    void Put(const uint256& hash, std::shared_ptr<CAuxPow> auxpow)
    {
        LOCK(m_mutex);
        if (m_map.emplace(hash, std::move(auxpow)).second) {
...
AuxpowRelayCache g_auxpow_relay_cache;
} // namespace

/** Build the header to place in a `headers` message for a given block index.
 *  Merge-mined (AuxPoW) blocks must carry their auxpow proof so the receiving peer can
 *  verify proof-of-work at the header layer (HasValidProofOfWork). The auxpow is not held
 *  in the block index; it is served from g_auxpow_relay_cache and, on a miss, read from the
 *  full block on disk (and cached). Returns false if the proof cannot be provided (e.g. the
 *  body is pruned) — the caller must then NOT emit the header, because serializing an
 *  auxpow-flagged header with a null auxpow throws. */
static bool BuildRelayHeader(const node::BlockManager& blockman, const CBlockIndex* pindex, CBlock& out)
{
    out = pindex->GetBlockHeader(); // 6 header fields, null auxpow, empty vtx
    if (out.nVersion & BLOCK_VERSION_AUXPOW) {
        const uint256 hash{pindex->GetBlockHash()};
        if (auto cached = g_auxpow_relay_cache.Get(hash)) {
            out.auxpow = std::move(cached);
            return true;
        }
        CBlock full;
        if (!blockman.ReadBlock(full, *pindex) || !full.auxpow) {
            // Body unavailable (pruned, or not yet downloaded): we cannot serve an
            // auxpow header without its proof. The caller stops the headers list here;
            // the requesting peer fetches the remainder from a peer that has the body.
            LogDebug(BCLog::NET, "Cannot serve auxpow header for block %s: block body unavailable\n",
                     hash.ToString());
            return false;
        }
        g_auxpow_relay_cache.Put(hash, full.auxpow);
        out.auxpow = std::move(full.auxpow);
    }
    return true;
}
...
bool PeerManagerImpl::TryLowWorkHeadersSync(Peer& peer, CNode& pfrom, const CBlockIndex* chain_start_header, std::vector<CBlockHeader>& headers)
{
    ...
    if (total_work < minimum_chain_work) {
        ...
        // (at least a full batch; a legacy peer may send a larger batch);
                                            ...
                                            bool via_compact_block)
{
    ...
    if (nCount >= m_opts.max_headers_result && !have_headers_sync) {
        // Headers message was at least a full batch; the peer may have more headers.
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        {
            CBlock relay_header;
            // Attach the auxpow proof for merge-mined blocks (served from the relay cache,
            // else read from the full block on disk and cached). We deliberately do NOT
            // truncate this response on a per-request read budget or serialized-size bound:
            // a headers batch shorter than max_headers_result is interpreted by the
            // requester as end-of-chain and would abort its IBD headers sync. The cache
            // bounds the cost of repeated requests; an honest IBD peer legitimately needs
            // these same reads. If a body is unavailable (pruned) we stop, as before,
            // rather than emit a header that would throw.
            if (!BuildRelayHeader(m_chainman.m_blockman, pindex, relay_header)) break;
            vHeaders.push_back(std::move(relay_header));
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        // The compact block now carries an auxpow proof (a transaction) for merge-mined
        // blocks; penalise a malformed one rather than letting it throw uncaught
        // (mirrors ESF-AUXPOW-002 on the `block` message).
        try {
            vRecv >> cmpctblock;
        } catch (const std::ios_base::failure& e) {
            Misbehaving(*peer, strprintf("malformed cmpctblock: %s", e.what()));
            return;
        }
        ...
        if (!prev_block) {
            ...
            return;
        } else if (prev_block->nChainWork + CalculateClaimedHeadersWork({{cmpctblock.GetBlockHeader()}}) < GetAntiDoSWorkThreshold()) {
        ...
        // The cmpctblock path admits the header with min_pow_checked=true, which
        // bypasses the AcceptBlockHeader anti-DoS gate; and for auxpow-flagged headers
        // CheckBlockHeader skips the PoW check entirely. Without a check here a forged
        // auxpow header (absent/under-worked parent proof) would be written into the
        // block index at zero cost and could poison m_best_header. HasValidProofOfWork
        // verifies the auxpow proof (parent-chain PoW); reject and penalise otherwise.
        if (!HasValidProofOfWork({cmpctblock.GetBlockHeader()}, m_chainparams.GetConsensus())) {
            Misbehaving(*peer, "cmpctblock header with invalid proof of work");
            return;
        }

        const CBlockIndex *pindex = nullptr;
        BlockValidationState state;
        if (!m_chainman.ProcessNewBlockHeaders({{cmpctblock.GetBlockHeader()}}, /*min_pow_checked=*/true, state, &pindex)) {
            if (state.IsInvalid()) {
        ...
        if (fRevertToHeaderProcessing) {
            ...
            return ProcessHeadersMessage(pfrom, *peer, {cmpctblock.GetBlockHeader()}, /*via_compact_block=*/true);
        }
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        if (nCount > m_opts.max_headers_result_accept) {
            Misbehaving(*peer, strprintf("headers message size = %u", nCount));
            return;
        }
        headers.resize(nCount);
        // A header may now carry an auxpow proof (which contains a transaction), so a
        // malformed one can throw during deserialization. Penalise the peer instead of
        // letting it propagate uncaught (mirrors ESF-AUXPOW-002 on the `block` message).
        // The auxpow's coinbase tx needs transaction serialization params, and the
        // HEADERS send path uses TX_WITH_WITNESS(vHeaders); read each header through a
        // matching TX_WITH_WITNESS view so a merge-mined header parses correctly.
        try {
            ...
            Misbehaving(*peer, strprintf("malformed headers: %s", e.what()));
            return;
        }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        std::shared_ptr<CBlock> pblock = std::make_shared<CBlock>();
        // ESF-AUXPOW-002: catch deserialization failure and penalise the peer.
        // A malformed CBlock (e.g. truncated/corrupt AuxPoW data) previously
        // propagated as an uncaught exception to ProcessMessages, which only logged
        // it; the peer received no misbehavior score and could flood without cost.
        try {
            vRecv >> TX_WITH_WITNESS(*pblock);
        } catch (const std::ios_base::failure& e) {
            Misbehaving(*peer, strprintf("malformed block: %s", e.what()));
            return;
        }
            ...
            if (!fRevertToInv) {
                ...
                for (const uint256& hash : peer->m_blocks_for_headers_relay) {
                    ...
                    if (fFoundStartingHeader) {
                        ...
                        CBlock relay_header;
                        if (!BuildRelayHeader(m_chainman.m_blockman, pindex, relay_header)) { fRevertToInv = true; break; }
                        vHeaders.push_back(std::move(relay_header));
