...
#include <config/elevenseventyfive-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatElevenSeventyFiveURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(ElevenSeventyFiveUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatElevenSeventyFiveURI(info));
}
