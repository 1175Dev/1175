This directory contains the source code for the ElevenSeventyFive Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### elevenseventyfivegui.(h/cpp)

- Represents the main window of the ElevenSeventyFive UI.
...
- ClientModel is used by the main application `elevenseventyfivegui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a elevenseventyfive:... link from a browser).
...
* UI elements like ElevenSeventyFiveAmountField, which inherit from QWidget.
* `elevenseventyfivestrings.cpp`: automatically generated
* `elevenseventyfiveunits.(h/cpp)`: ESF / mESF / etc. handling
* `callback.h`
...
**Note:** Do not change `local/elevenseventyfive_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `elevenseventyfive-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `elevenseventyfive-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `elevenseventyfive-qt`
9. You're all set! Start developing, building, and debugging the ElevenSeventyFive Core GUI
