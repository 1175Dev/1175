                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<ElevenSeventyFiveAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = ElevenSeventyFiveUnits::formatWithUnit(unit, expected_balance, false, ElevenSeventyFiveUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_elevenseventyfive-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_elevenseventyfive-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_elevenseventyfive-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    ElevenSeventyFiveAmountField* amountInput = receiveCoinsDialog.findChild<ElevenSeventyFiveAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("elevenseventyfive:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_elevenseventyfive-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
