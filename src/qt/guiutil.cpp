...
#include <qt/elevenseventyfiveaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ElevenSeventyFive address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ElevenSeventyFiveAddressEntryValidator(parent));
    widget->setCheckValidator(new ElevenSeventyFiveAddressCheckValidator(parent));
}
...
bool parseElevenSeventyFiveURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no elevenseventyfive: URI
    if(!uri.isValid() || uri.scheme() != QString("elevenseventyfive"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!ElevenSeventyFiveUnits::parse(ElevenSeventyFiveUnit::ESF, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseElevenSeventyFiveURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseElevenSeventyFiveURI(uriInstance, out);
}

QString formatElevenSeventyFiveURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("elevenseventyfive:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ElevenSeventyFiveUnits::format(ElevenSeventyFiveUnit::ESF, info.amount, false, ElevenSeventyFiveUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openElevenSeventyFiveConf()
{
    ...
    /* Open elevenseventyfive.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ElevenSeventyFive.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ElevenSeventyFive (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("ElevenSeventyFive (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for ElevenSeventyFive*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "elevenseventyfive.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("elevenseventyfive-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a elevenseventyfive.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=ElevenSeventyFive\n";
        else
            optionFile << strprintf("Name=ElevenSeventyFive (%s)\n", chain);
