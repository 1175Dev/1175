...
/** Call node.updateRwSetting() with ElevenSeventyFive 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because ElevenSeventyFive 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/elevenseventyfive/elevenseventyfive/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because elevenseventyfive 22.x will not try to read these.
...
//! Convert enabled/size values to elevenseventyfive -prune setting.
...
//! Get pruning enabled value to show in GUI from elevenseventyfive -prune setting.
...
//! Get pruning size value to show in GUI from elevenseventyfive -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayElevenSeventyFiveUnit")) {
        settings.setValue("DisplayElevenSeventyFiveUnit", QVariant::fromValue(ElevenSeventyFiveUnit::ESF));
    }
    QVariant unit = settings.value("DisplayElevenSeventyFiveUnit");
    if (unit.canConvert<ElevenSeventyFiveUnit>()) {
        m_display_elevenseventyfive_unit = unit.value<ElevenSeventyFiveUnit>();
    } else {
        m_display_elevenseventyfive_unit = ElevenSeventyFiveUnit::ESF;
        settings.setValue("DisplayElevenSeventyFiveUnit", QVariant::fromValue(m_display_elevenseventyfive_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if elevenseventyfive.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_elevenseventyfive_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ElevenSeventyFiveUnit>() == m_display_elevenseventyfive_unit) return;
    m_display_elevenseventyfive_unit = new_unit.value<ElevenSeventyFiveUnit>();
    QSettings settings;
    settings.setValue("DisplayElevenSeventyFiveUnit", QVariant::fromValue(m_display_elevenseventyfive_unit));
    Q_EMIT displayUnitChanged(m_display_elevenseventyfive_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/elevenseventyfive/elevenseventyfive/pull/8273
    ...
    // (https://github.com/elevenseventyfive-core/gui/issues/567).
