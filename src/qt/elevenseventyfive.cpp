...
#include <qt/elevenseventyfive.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<ElevenSeventyFiveUnit>("ElevenSeventyFiveUnit");
#else
    qRegisterMetaType<ElevenSeventyFiveUnit>("ElevenSeventyFiveUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. elevenseventyfive_de.qm (shortcut "de" needs to be defined in elevenseventyfive.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. elevenseventyfive_de_DE.qm (shortcut "de_DE" needs to be defined in elevenseventyfive.qrc)
...
static const char* qt_argv = "elevenseventyfive-qt";

ElevenSeventyFiveApplication::ElevenSeventyFiveApplication()
...
void ElevenSeventyFiveApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ElevenSeventyFiveApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ElevenSeventyFiveGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ElevenSeventyFiveApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ElevenSeventyFiveApplication::createOptionsModel(bool resetSettings)
{
...
void ElevenSeventyFiveApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ElevenSeventyFiveGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ElevenSeventyFiveGUI::quitRequested, this, &ElevenSeventyFiveApplication::requestShutdown);
...
void ElevenSeventyFiveApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ElevenSeventyFiveApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ElevenSeventyFiveApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ElevenSeventyFiveApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ElevenSeventyFiveApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ElevenSeventyFiveApplication::handleRunawayException);
    connect(this, &ElevenSeventyFiveApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ElevenSeventyFiveApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ElevenSeventyFiveApplication::parameterSetup()
{
...
void ElevenSeventyFiveApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ElevenSeventyFiveApplication::requestInitialize()
{
...
void ElevenSeventyFiveApplication::requestShutdown()
{
...
void ElevenSeventyFiveApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ElevenSeventyFiveGUI::handlePaymentRequest);
            connect(window, &ElevenSeventyFiveGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ElevenSeventyFiveApplication::handleRunawayException(const QString &message)
{
...
void ElevenSeventyFiveApplication::handleNonFatalException(const QString& message)
{
...
WId ElevenSeventyFiveApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ElevenSeventyFiveApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ElevenSeventyFiveGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(elevenseventyfive);
    Q_INIT_RESOURCE(elevenseventyfive_locale);
    ...
    ElevenSeventyFiveApplication app;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see elevenseventyfive-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see elevenseventyfive-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
