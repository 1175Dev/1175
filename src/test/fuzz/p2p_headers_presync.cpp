...
public:
    HeadersSyncSetup(const ChainType chain_type, TestOpts opts) : TestingSetup(chain_type, opts)
    {
        ...
        // Keep the accept bound in step with the (reduced) send bound so this fuzz
        // target still rejects anything larger than a single FUZZ_MAX_HEADERS_RESULTS batch.
        peerman_opts.max_headers_result_accept = FUZZ_MAX_HEADERS_RESULTS;
