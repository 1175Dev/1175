    ...
    : m_args{}
{
    ...
    // Defensive reset: a prior test that aborted in its body (e.g. hit an assert on a
    // hardcoded value that differs on 1175) skips this fixture's destructor, leaving gArgs
    // populated and the test logger connected. Without resetting, this test's
    // SetupServerArgs would assert on duplicate args and StartLogging on !m_buffering,
    // cascading SIGABRTs across the rest of a single-process run. Reset both first so an
    // aborting test fails in isolation. (No-op when the previous test cleaned up normally.)
...
{
    // 1175: the mock clock must be AFTER the regtest genesis (nTime 1769468775, ~2026-01-26).
    // Bitcoin's 1598887952 (2020) predates the 1175 genesis, which forced every mined block's
    // timestamp to genesis+1 — far in the "future" of the 2020 clock — aborting the fixture
    // (time-too-new) and cascading SIGABRT across every test that uses this setup.
    SetMockTime(1769470000);
    ...
    {
        LOCK(::cs_main);
        // 1175: recomputed for the 1175 regtest chain — its genesis and coinbase differ from
        // Bitcoin's, so the deterministic 100-block tip differs from upstream's 571d80a9….
        assert(
            m_node.chainman->ActiveChain().Tip()->GetBlockHash().ToString() ==
            "027c34e3d348c503d39d0277f2a5c774679858ec1fd839385fd6151c37f32f2e");
    }
}
