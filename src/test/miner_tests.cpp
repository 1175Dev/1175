...
constexpr static struct {
    ...
    unsigned int nonce;
} BLOCKINFO[]{{8, 583256678},  {0, 971478989},  {2, 1169652563},  {6, 67006287},  {7, 431025477},  {8, 81037995},
              {8, 208050031},  {2, 1951908337},  {4, 215419498},  {1, 493006998},  {8, 1284252787},  {4, 639684601},
              {3, 249472283},  {8, 1161775671},  {6, 399736959},  {5, 395029708},  {5, 1097103267},  {4, 966560430},
              {0, 729749037},  {5, 318712360},  {3, 167550227},  {2, 274750787},  {2, 255743583},  {7, 562302830},
              {2, 269326770},  {0, 403012100},  {1, 221677098},  {6, 539012573},  {7, 394813008},  {4, 815444785},
              {7, 507791176},  {6, 468633238},  {3, 926607455},  {2, 201231060},  {3, 169905015},  {8, 432635136},
              {5, 774031018},  {3, 1195993395},  {0, 434412002},  {3, 298190780},  {0, 559102183},  {2, 501783303},
              {3, 530258499},  {2, 475258975},  {8, 231247474},  {2, 496722562},  {4, 357625399},  {8, 63136294},
              {7, 641364617},  {3, 480749822},  {8, 1424917258},  {4, 753490498},  {1, 288697654},  {6, 669948439},
              {5, 1901660892},  {3, 558127714},  {3, 1122354596},  {0, 546458795},  {8, 189723570},  {5, 252821400},
              {0, 199396547},  {6, 559076246},  {6, 1657006966},  {6, 815278324},  {6, 719271209},  {5, 544093044},
              {0, 340338884},  {6, 851454570},  {4, 497909188},  {8, 525574946},  {6, 574741460},  {6, 1646718843},
              {6, 358014051},  {5, 1648055190},  {8, 1103039253},  {7, 556871034},  {3, 1139798020},  {5, 305668404},
              {2, 361571900},  {2, 144063439},  {6, 202041055},  {7, 424248691},  {4, 575412832},  {3, 1414674344},
              {4, 876835944},  {0, 344636632},  {6, 54783652},  {3, 181348570},  {5, 173608309},  {8, 517584486},
              {4, 1228609808},  {8, 326207450},  {6, 550382869},  {0, 876441198},  {7, 412643362},  {7, 717411746},
              {2, 223832901},  {6, 252498434},  {7, 844352507},  {7, 195003460},  {4, 97412367},  {1, 925662588},
              {0, 795349088},  {6, 679765528},  {6, 773676857},  {5, 186780178},  {6, 884877084},  {7, 397227647},
              {8, 255238351},  {0, 456768512}};
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    // 1175: CSV/BIP68 is active from genesis on 1175 mainnet (CSVHeight=0), whereas Bitcoin
    // activates it at height 419328. This mainnet test runs at height ~110, so upstream relies on
    // CSV being INACTIVE here (see the first createNewBlock below).
    const bool csv_active{m_node.chainman->GetConsensus().CSVHeight <= m_node.chainman->ActiveChain().Tip()->nHeight + 1};
    ...
    // None of the absolute height/time locked txs make it into the template because createNewBlock
    // still checks IsFinalTx. The relative-locked txs are non-final under BIP68, and the block
    // assembler only filters IsFinalTx (not sequence locks), so it still tries to include them:
    //  - CSV inactive (Bitcoin, at these low mainnet heights): the template is valid and includes
    //    the two relative-locked txs (coinbase + 2 = 3).
    //  - CSV active (1175, from genesis): a non-final tx cannot be mined yet, so createNewBlock's
    //    template validity check rejects the block. (In normal operation the mempool would never
    //    have accepted these non-final txs; the test adds them directly to force this state.)
    if (csv_active) {
