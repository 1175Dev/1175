    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // AuxPoW blocks' own hash never satisfies the target; only the parent
                // block hash does. Skip CheckProofOfWork for post-activation AuxPoW
                // blocks — PoW was verified at acceptance time and the stored index is
                // authoritative. The activation-height guard prevents a tampered index
                // entry for a pre-activation block from bypassing the check via the
                // BLOCK_VERSION_AUXPOW bit.
                const bool isAuxpow = (pindexNew->nVersion & BLOCK_VERSION_AUXPOW) &&
                                      consensusParams.IsAuxpowActive(pindexNew->nHeight);
                if (!isAuxpow &&
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos) const
{
    ...
    // Check the header.  For AuxPoW blocks the parent block hash (not the
    // child hash) must satisfy PoW; CheckAuxProofOfWork handles both cases.
