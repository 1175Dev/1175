...
const char * const BITCOIN_CONF_FILENAME = "elevenseventyfive.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\ElevenSeventyFive
    //   new: C:\Users\Username\AppData\Local\ElevenSeventyFive
    // macOS: ~/Library/Application Support/ElevenSeventyFive
    // Unix-like: ~/.elevenseventyfive
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "ElevenSeventyFive";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "ElevenSeventyFive";
#else
    fs::path pathRet;
